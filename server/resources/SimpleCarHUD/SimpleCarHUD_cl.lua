...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if (IsPedInAnyVehicle(player, false)) then
            ...
            drawTxt(("%.3d"):format(math.ceil(speed)), 2, speedColor, 0.4, screenPosX + 0.010, screenPosY + 0.024)
            drawTxt("KMH", 2, speedColorText, 0.3, screenPosX + 0.035, screenPosY + 0.028)
            ...
            drawTxt(("%.3d"):format(math.ceil(currentFuel)), 2, fuelColor, 0.4, screenPosX + 0.060, screenPosY + 0.024)
            drawTxt("FUEL", 2, fuelColorText, 0.3, screenPosX + 0.080, screenPosY + 0.028)
 
            -- Get time and display
            local hour = GetClockHours()
            local minute = GetClockMinutes()
            local timeText = ("%.2d"):format((hour == 0) and 12 or hour) .. ":" .. ("%.2d"):format( minute) .. ((hour < 12) and " AM" or " PM")
            drawTxt(timeText, 4, {255,255,255}, 0.3, screenPosX + 0.010, screenPosY + 0.052)
 
            -- Draw cruise control status
            cruiseColor = cruiseIsOn and cruiseColorOn or cruiseColorOff
            drawTxt("CRUISE", 2, cruiseColor, 0.3, screenPosX + 0.040, screenPosY + 0.052)
 
            -- Draw seatbelt status
            seatbeltColor = seatbeltIsOn and seatbeltColorOn or seatbeltColorOff
            drawTxt("SEATBELT", 2, seatbeltColor, 0.3, screenPosX + 0.070, screenPosY + 0.052)
            ...
            drawTxt(locationText, 4, locationColorText, 0.3, screenPosX + 0.010, screenPosY + 0.074)
        else
